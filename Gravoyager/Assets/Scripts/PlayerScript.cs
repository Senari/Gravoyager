...
public class PlayerScript : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        distancePerTime = Vector2.Distance(oldPosition, myPosition); //float value
		velocityVector = oldPosition - myPosition; //vector value
    ...
    {	
		...
		Vector2 mov;
            ...
            for (int d = 0; d < trajectoryDots; d++)
            {
                ...
                //futurePlayer.transform.position = gravity - velocityVector; //Location of future me
                ...
                {
                    // exclude yourself
				if (/*(body[i] != futurePlayer) &&*/ (body[i] != gameObject))
                    {
					...
					gravity = body[i].transform.position - this.transform.position;
					mov = new Vector2((gravity.normalized.x * (weight/playerMass)) + velocityVector.normalized.x * distancePerTime, (gravity.normalized.y* (weight/playerMass)) + velocityVector.normalized.y * distancePerTime);
						futurePlayer.transform.position = pos2 - mov;
						
						//print ("GravitySpeed: ");
	                 }
                }		
			futurePlayer.GetComponent<Renderer>().enabled = true;
				
				//futurePlayer.transform.position = 
